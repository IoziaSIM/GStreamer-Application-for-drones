    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_NUM_SRC_PADS: {  
      nsrcpads = g_value_get_int (value);     //store the number of channels
      // array for the control of number of RTCP packet arrived
      buf = g_new0 (gint32, nsrcpads);    
      bin = g_new0 (gint, nsrcpads);
      break;
    }  
    /* end changes */
    default:
    ...
    GstBuffer * buffer)
{
  ...
  gst_rtcp_buffer_get_first_packet (&rtcp, &packet);  //init packet
  gst_rtcp_buffer_unmap (&rtcp);
  if(packet.count > 0) {  // only Receiver Report in the sender
    gst_rtcp_packet_get_rb(&packet, 0, &ssrc, &fractionlost, &packetslost, &exthighestseq, &jitter, &lsr, &dlsr);
    buf[ssrc-x] = packetslost;   // save packet loss of channel with index "ssrc-x"
    bin[ssrc-x] = 1;       // save that the RTCP packet for channel "ssrc-x" is arrived
    
    // if each channel have its report, update the file txt
    ...
    if(check == TRUE) {
      GST_LOG_OBJECT (rtpsession, "Updating file .txt!");
      if ((ploss = fopen("path/ploss.txt", "w")) == NULL)
		    GST_LOG_OBJECT (rtpsession, "Creation file .txt failed!");
      for (gint i = 0; i < nsrcpads; i++) {
        fprintf(ploss, "%d\n", buf[i]);    // update the number of packet loss in the file txt
        bin[i]=0;   
      }
      fclose (ploss);
    }
