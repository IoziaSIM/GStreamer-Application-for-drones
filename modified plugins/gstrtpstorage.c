...
/* changes */
guint16 seqnum_prec;    // seqnum of previous packet 
gint count = 0;     // number of the packet
guint16 agg_ploss[1];      // array for the UDP socket
/* end changes */
...
gst_rtp_storage_chain (GstPad * pad, GstObject * parent, GstBuffer * buf)
{
  ...
  seqnum_curr = gst_rtp_buffer_get_seq (&rtp);    // save seqnum of this packet 
  gst_rtp_buffer_unmap (&rtp);

  count ++;

  // if the packet is the absolute first 
  if(count == 1)
    seqnum_prec = seqnum_curr;
  else {
    // if the packet is not the next of the previous
    if(seqnum_curr != seqnum_prec + 1)
      agg_ploss[0] += 1;      // the complessive number of packet loss 

    seqnum_prec = seqnum_curr;   // update data for the next packet
  }
  ...
  if (count % 50 == 0) {
    if ((sockfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
      ...
      controller.sin_port = htons(12000);
      controller.sin_addr.s_addr = inet_addr("127.0.0.1");  //remote IP
      // send the information 
      sendto(sockfd, (guint16 *) agg_ploss, sizeof(agg_ploss), 0, (const struct sockaddr *) &controller, sizeof(controller));
      close(sockfd);
    }
  }
  /* end changes */
