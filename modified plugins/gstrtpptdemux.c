...
gst_rtp_pt_demux_chain (GstPad * pad, GstObject * parent, GstBuffer * buf)
{
  ...
  GstPad *srcpad;
  GstCaps *caps;
  GstRTPBuffer rtp = GST_RTP_BUFFER_INIT; //change
  ...
  ext = (guint32*) data;
  gst_rtp_buffer_set_ssrc (&rtp, ext[0]); // setto lo stesso SSRC per tutti i pacchetti
  gst_rtp_buffer_set_seq (&rtp, ext[1]);   //rimetto il seqnum iniziale
 
  
  //g_print("ssrc %d, seq %d\n", ext[0], ext[1]);
