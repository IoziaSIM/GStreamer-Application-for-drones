...
gst_rtp_pt_demux_chain (GstPad * pad, GstObject * parent, GstBuffer * buf)
{
  ...
  GstPad *srcpad;
  GstCaps *caps;
  GstRTPBuffer rtp = GST_RTP_BUFFER_INIT;  //changes
  ...
  gpointer data;
  guint size;
  guint32* ext = g_new (guint32, 2);
  buf = gst_buffer_make_writable(buf);
  
  if (!gst_rtp_buffer_map (buf, GST_MAP_READWRITE, &rtp))
    goto invalid_buffer;

  gst_rtp_buffer_get_extension_onebyte_header (&rtp, 5, 0, &data, &size);
  ext = (guint32*) data;      // save data of header extension in this array
  gst_rtp_buffer_set_ssrc (&rtp, ext[0]);    // set original SSRC
  gst_rtp_buffer_set_seq (&rtp, ext[1]);   // set original seqnum
